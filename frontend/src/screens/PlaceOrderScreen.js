...
function PlaceOrderScreen({ history }) {
    ...
    cart.itemPrice = cart.cartItems.reduce((acc, item) => acc + item.price * item.qty, 0).toFixed(2);  // tong so tien tat ca san pham

    cart.shippingPrice = (cart.itemPrice > 1580000.00 ? 0 : 10).toFixed(2);
    cart.taxPrice = Number((0.082) * cart.itemPrice).toFixed(0);
